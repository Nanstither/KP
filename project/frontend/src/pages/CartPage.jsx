...
import { Trash2, Edit3, ArrowRight, Package, LogIn, UserPlus, X, List } from "lucide-react";
import api from "@/services/api";
import { useAuth } from "@/context/AuthContext";
import BuildComponentsModal from "@/components/BuildComponentsModal";
import { cartItemComponentsToList, prebuiltComponentsToList } from "@/lib/buildComponents";
...
export default function CartPage() {
  ...
  const openComponentsModal = async (item) => {
    if (item.type === "component") {
      const compId = item.components?.[0]?.component_id || item.components?.[0]?.component?.id;
      if (compId) {
    ...
    if (item.type === "prebuilt" && item.prebuilt_id) {
      try {
        ...
        const pc = (res.data || []).find((p) => p.id === item.prebuilt_id);
        if (pc?.components) {
          setModalComponents(prebuiltComponentsToList(pc.components));
        }
      } catch (err) {
        console.error("Ошибка загрузки компонентов ПК:", err);
      }
    } else if (item.type === "custom") {
      setModalComponents(cartItemComponentsToList(item));
    }

    setComponentsModalOpen(true);
  };

  const showSpecsButton = (item) => item.type === "custom" || item.type === "prebuilt";
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f0f10] text-gray-800 dark:text-gray-200 pt-24 pb-12 px-4">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cart.items.map((item, index) => {
                ...
                const displayImage = itemImage
                  ? `${STORAGE_URL}/${itemImage}`
                  : placeholderImage;
      ...
      <BuildComponentsModal
        isOpen={componentsModalOpen && !!selectedItem}
        onClose={() => {
        ...
        subtitle={
          selectedItem?.type === "prebuilt"
            ? "Готовый компьютер"
            : "Пользовательская сборка"
        }
        components={modalComponents}
        totalPrice={selectedItem?.total_price}
      />
    </div>
  );
}