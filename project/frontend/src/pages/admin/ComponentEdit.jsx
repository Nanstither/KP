...
const Field = ({ label, type = 'text', val, set, dis, children, placeholder }) => (
  <div className="space-y-1">
    ...
    {children || (
      <input type={type} value={val ?? ''} onChange={e => set(type === 'number' ? Number(e.target.value) : e.target.value)} 
        disabled={dis} placeholder={placeholder || ''} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 placeholder:text-gray-600 focus:border-purple-500 focus:outline-none transition-colors" />
    )}
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-[#0f0f10] text-gray-200 p-6 pt-24">
      <div className="max-w-6xl mx-auto space-y-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-gray-400 hover:text-white mb-2"><ArrowLeft className="w-4 h-4" /> Назад в таблицу</button>

        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div><h1 className="text-2xl font-bold text-white">{data.component.model}</h1><p className="text-sm text-gray-500 mt-1">Категория: <span className="text-purple-400">{categoryName}</span></p></div>
          {!isEditing ? (
            <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-lg transition-colors"><Edit className="w-4 h-4" /> Редактировать</button>
          ) : (
            <div className="flex gap-2">
              <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg disabled:opacity-50 transition-colors">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Сохранить</button>
              <button onClick={handleCancel} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-gray-300 px-4 py-2 rounded-lg transition-colors"><X className="w-4 h-4" /> Отмена</button>
            </div>
          )}
        </div>
        ...
        <div className="bg-[#141416] border border-white/10 rounded-xl p-6">
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="w-full md:w-80 flex-shrink-0 space-y-4">
              <div className={`aspect-square bg-[#0a0a0c] border border-white/10 rounded-xl overflow-hidden relative group transition-all ${isEditing ? 'cursor-pointer hover:border-purple-500' : ''}`} onClick={() => isEditing && fileInputRef.current?.click()}>
                ...
                {isEditing && <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Upload className="w-6 h-6 text-white drop-shadow-md" /></div>}
                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={e => setImageFile(e.target.files[0])} />
              </div>
              <div className="bg-[#0a0a0c] border border-white/10 rounded-lg p-3 text-center">
                <p className="text-xs text-gray-500 mb-1">Файл будет сохранён автоматически по пути:</p>
                <p className="text-xs font-mono text-gray-300 truncate" title={autoPath}>{autoPath}</p>
              </div>
              {isEditing && (
                <div className="relative">
                  <button onClick={() => setShowTemplates(!showTemplates)} className="w-full flex items-center justify-center gap-2 text-xs text-gray-400 hover:text-purple-300 transition-colors py-2">
                    <ImageIcon className="w-3 h-3" /> Выбрать из шаблона <ChevronDown className={`w-3 h-3 transition-transform ${showTemplates ? 'rotate-180' : ''}`} />
                  </button>
                  {showTemplates && <div className="absolute top-full left-0 right-0 mt-1 bg-[#1a1a1c] border border-white/10 rounded-lg shadow-xl z-10 p-2 max-h-48 overflow-y-auto">{templates.length > 0 ? templates.map(t => <button key={t.path} onClick={() => selectTemplate(t.path)} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-300 hover:bg-purple-600/20 hover:text-purple-300 rounded-md transition-colors text-left"><div className="w-8 h-8 bg-[#0a0a0c] rounded border border-white/10 overflow-hidden flex-shrink-0"><img src={t.path} className="w-full h-full object-cover" /></div>{t.label}</button>) : <p className="px-3 py-2 text-xs text-gray-500">Нет шаблонов</p>}</div>}
                </div>
              )}
            </div>
            ...
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Field label="Бренд"><select value={base.brand_id} onChange={e => setBase({ ...base, brand_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите бренд...</option>{refs.brands?.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></Field>
        ...
        <div className="bg-[#141416] border border-white/10 rounded-xl p-6 space-y-4">
          ...
          {slug === 'cpu' && <>
            <Field label="Сокет"><select value={spec.socket_id || ''} onChange={e => setSpec({ ...spec, socket_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите сокет...</option>{refs.sockets?.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field>
          ...
          {slug === 'gpu' && <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <Field label="Тип VRAM"><select value={spec.vram_type_id || ''} onChange={e => setSpec({ ...spec, vram_type_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите тип...</option>{refs.vram_types?.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></Field>
          ...
          {slug === 'ram' && <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <Field label="Тип ОЗУ"><select value={spec.ram_type_id || ''} onChange={e => setSpec({ ...spec, ram_type_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите тип...</option>{refs.ram_types?.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></Field>
          ...
          {slug === 'motherboard' && <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <Field label="Сокет"><select value={spec.socket_id || ''} onChange={e => setSpec({ ...spec, socket_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите сокет...</option>{refs.sockets?.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field>
              <Field label="Форм-фактор"><select value={spec.form_factor_id || ''} onChange={e => setSpec({ ...spec, form_factor_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите форм-фактор...</option>{refs.form_factors?.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}</select></Field>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Field label="Тип RAM"><select value={spec.ram_type_id || ''} onChange={e => setSpec({ ...spec, ram_type_id: +e.target.value })} disabled={!isEditing} className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-white disabled:opacity-70 focus:border-purple-500 focus:outline-none"><option value="" disabled>Выберите тип...</option>{refs.ram_types?.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></Field>
          ...
          {slug === 'cooler' && <>
            ...
            <div className="space-y-2 pt-2"><label className="text-xs text-gray-400 uppercase">Совместимые сокеты</label><div className="flex flex-wrap gap-2">{refs.sockets?.map(s => { const isSelected = (spec.compatible_sockets || []).includes(s.id); return <button key={s.id} type="button" onClick={() => { const current = spec.compatible_sockets || []; const updated = isSelected ? current.filter(id => id !== s.id) : [...current, s.id]; setSpec({ ...spec, compatible_sockets: updated }); }} disabled={!isEditing} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${isSelected ? 'bg-purple-600/20 border-purple-500 text-purple-300' : 'bg-[#0a0a0c] border-white/10 text-gray-400 hover:border-white/20'} ${!isEditing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>{s.name}</button> })}</div></div>
          </>}
          {slug === 'case' && <>
             <div className="space-y-4">
               <div className="space-y-2"><label className="text-xs text-gray-400 uppercase">Тип корпуса</label><div className="flex flex-wrap gap-2">{refs.case_types?.map(t => <button key={t.id} type="button" onClick={() => setSpec({ ...spec, case_type_id: t.id })} disabled={!isEditing} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${spec.case_type_id === t.id ? 'bg-purple-600/20 border-purple-500 text-purple-300' : 'bg-[#0a0a0c] border-white/10 text-gray-400 hover:border-white/20'} ${!isEditing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>{t.name}</button>)}</div></div>
               <div className="space-y-2"><label className="text-xs text-gray-400 uppercase">Материал</label><div className="flex flex-wrap gap-2">{refs.materials?.map(m => <button key={m.id} type="button" onClick={() => setSpec({ ...spec, material_id: m.id })} disabled={!isEditing} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${spec.material_id === m.id ? 'bg-purple-600/20 border-purple-500 text-purple-300' : 'bg-[#0a0a0c] border-white/10 text-gray-400 hover:border-white/20'} ${!isEditing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>{m.name}</button>)}</div></div>
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2"><Field label="Слоты сверху" type="number" val={spec.top_fan_slots} set={v => setSpec({ ...spec, top_fan_slots: +v })} dis={!isEditing} placeholder="2" /><Field label="Вентиляторов" type="number" val={spec.fans_included} set={v => setSpec({ ...spec, fans_included: +v })} dis={!isEditing} placeholder="3" /><Field label="Отсеки 3.5" type="number" val={spec.drive_bays_3_5} set={v => setSpec({ ...spec, drive_bays_3_5: +v })} dis={!isEditing} placeholder="2" /><Field label="Отсеки 2.5" type="number" val={spec.drive_bays_2_5} set={v => setSpec({ ...spec, drive_bays_2_5: +v })} dis={!isEditing} placeholder="4" /><Field label="USB-A" type="number" val={spec.front_usb_a} set={v => setSpec({ ...spec, front_usb_a: +v })} dis={!isEditing} placeholder="2" /><Field label="USB-C" type="number" val={spec.front_usb_c} set={v => setSpec({ ...spec, front_usb_c: +v })} dis={!isEditing} placeholder="1" /></div>
               <div className="space-y-2 pt-2"><label className="text-xs text-gray-400 uppercase">Поддерживаемые форм-факторы</label><div className="flex flex-wrap gap-2">{refs.form_factors?.map(f => { const isSelected = (spec.supported_form_factors || []).includes(f.id); return <button key={f.id} type="button" onClick={() => { const current = spec.supported_form_factors || []; const updated = isSelected ? current.filter(id => id !== f.id) : [...current, f.id]; setSpec({ ...spec, supported_form_factors: updated }); }} disabled={!isEditing} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${isSelected ? 'bg-purple-600/20 border-purple-500 text-purple-300' : 'bg-[#0a0a0c] border-white/10 text-gray-400 hover:border-white/20'} ${!isEditing ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>{f.name}</button> })}</div></div>
             </div>
          </>}
        </div>
      </div>
