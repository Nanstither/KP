...
const chipClass = (selected, error) =>
...
const Field = ({ label, type = 'text', val, set, error, children, placeholder, onChange, step }) => (
...
export default function ComponentCreate() {
  ...
  const validateSpecs = () => {
    ...
    if (slug === 'storage') {
      if (!spec.type?.trim()) newSpecErrors.type = 'Выберите тип';
    ...
    if (slug === 'cooler') {
      ...
      if (!spec.type?.trim()) newSpecErrors.type = 'Выберите тип охлаждения';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f0f10] text-gray-800 dark:text-gray-200 p-6 pt-24 transition-colors">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Создание компонента</h1>
          ...
          <div className="flex gap-2">
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg disabled:opacity-50 transition-colors shadow-sm"
            >
            ...
            <button
              onClick={() => navigate(-1)}
              className="flex items-center gap-2 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg transition-colors border border-gray-200 dark:border-white/10"
            >
            ...
            <div className="w-full md:w-80 flex-shrink-0 space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs text-gray-500 dark:text-gray-400 uppercase">Изображение (необязательно)</label>
                  <span className="text-[10px] text-gray-400 dark:text-gray-600">Сохранится как:</span>
                </div>
            ...
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Field label="Бренд *" error={errors.brand_id} onChange={() => setErrors(p => ({ ...p, brand_id: '' }))}>
                  <select
                    value={base.brand_id}
                    onChange={e => { setBase({ ...base, brand_id: +e.target.value }); setErrors(p => ({ ...p, brand_id: '' })); }}
        ...
        <div className="bg-white dark:bg-[#141416] border border-gray-200 dark:border-white/10 rounded-xl p-6 space-y-4 shadow-sm dark:shadow-none">
          ...
          {slug === 'cpu' && (
            <>
              <Field label="Сокет *" error={specErrors.socket_id} onChange={() => setSpecErrors(p => ({ ...p, socket_id: '' }))}>
                <select
                  value={spec.socket_id || ''}
                  onChange={e => setSpec({ ...spec, socket_id: +e.target.value })}
                  className={selectClass(specErrors.socket_id)}
                >
          ...
          {slug === 'gpu' && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <Field label="Тип VRAM *" error={specErrors.vram_type_id} onChange={() => setSpecErrors(p => ({ ...p, vram_type_id: '' }))}>
                <select value={spec.vram_type_id || ''} onChange={e => setSpec({ ...spec, vram_type_id: +e.target.value })} className={selectClass(specErrors.vram_type_id)}>
          ...
          {slug === 'ram' && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <Field label="Тип ОЗУ *" error={specErrors.ram_type_id} onChange={() => setSpecErrors(p => ({ ...p, ram_type_id: '' }))}>
                <select value={spec.ram_type_id || ''} onChange={e => setSpec({ ...spec, ram_type_id: +e.target.value })} className={selectClass(specErrors.ram_type_id)}>
                  <option value="">Выберите тип...</option>
          ...
          {slug === 'motherboard' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Field label="Сокет *" error={specErrors.socket_id} onChange={() => setSpecErrors(p => ({ ...p, socket_id: '' }))}>
                  <select value={spec.socket_id || ''} onChange={e => setSpec({ ...spec, socket_id: +e.target.value })} className={selectClass(specErrors.socket_id)}>
                    <option value="">Выберите сокет...</option>
                    {refs.sockets?.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                </Field>
                <Field label="Форм-фактор *" error={specErrors.form_factor_id} onChange={() => setSpecErrors(p => ({ ...p, form_factor_id: '' }))}>
                  <select value={spec.form_factor_id || ''} onChange={e => setSpec({ ...spec, form_factor_id: +e.target.value })} className={selectClass(specErrors.form_factor_id)}>
          ...
          {slug === 'storage' && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <Field label="Тип *" error={specErrors.type} onChange={() => setSpecErrors(p => ({ ...p, type: '' }))}>
                <select value={spec.type || ''} onChange={e => setSpec({ ...spec, type: e.target.value })} className={selectClass(specErrors.type)}>
                  <option value="">Выберите тип...</option>
          ...
          {slug === 'cooler' && (
            <>
              ...
              <div className="space-y-2 pt-2">
                  ...
                  {refs.sockets?.map(s => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          const c = spec.compatible_sockets || [];
                          setSpec({ ...spec, compatible_sockets: sel ? c.filter(i => i !== s.id) : [...c, s.id] });
                          setSpecErrors(p => ({ ...p, compatible_sockets: '' }));
                        }}
                        className={chipClass(sel, err)}
                      >
                        {s.name}
                      </button>
                    );
                ...
                {specErrors.compatible_sockets && <p className="text-xs text-red-500 dark:text-red-400">{specErrors.compatible_sockets}</p>}
              </div>
            </>
          )}
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {refs.case_types?.map(t => (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => { setSpec({ ...spec, case_type_id: t.id }); setSpecErrors(p => ({ ...p, case_type_id: '' })); }}
                      className={chipClass(spec.case_type_id === t.id, specErrors.case_type_id)}
                    >
                      {t.name}
                    </button>
                  ))}
                </div>
                {specErrors.case_type_id && <p className="text-xs text-red-500 dark:text-red-400">{specErrors.case_type_id}</p>}
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {refs.materials?.map(m => (
                    <button
                      key={m.id}
                      type="button"
                      onClick={() => { setSpec({ ...spec, material_id: m.id }); setSpecErrors(p => ({ ...p, material_id: '' })); }}
                      className={chipClass(spec.material_id === m.id, specErrors.material_id)}
                    >
                      {m.name}
                    </button>
                  ))}
                </div>
                {specErrors.material_id && <p className="text-xs text-red-500 dark:text-red-400">{specErrors.material_id}</p>}
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                <Field label="Слоты сверху" type="number" val={spec.top_fan_slots} set={v => setSpec({ ...spec, top_fan_slots: +v })} placeholder="2" />
                <Field label="Вентиляторов в комплекте" type="number" val={spec.fans_included} set={v => setSpec({ ...spec, fans_included: +v })} placeholder="3" />
                ...
                <Field label="USB-A (перед)" type="number" val={spec.front_usb_a} set={v => setSpec({ ...spec, front_usb_a: +v })} placeholder="2" />
                <Field label="USB-C (перед)" type="number" val={spec.front_usb_c} set={v => setSpec({ ...spec, front_usb_c: +v })} placeholder="1" />
                <Field label="Макс. длина GPU (мм)" type="number" val={spec.max_length_gpu} set={v => setSpec({ ...spec, max_length_gpu: +v })} placeholder="400" />
                <Field label="Высота (мм)" type="number" val={spec.height} set={v => setSpec({ ...spec, height: +v })} placeholder="450" />
                <Field label="Ширина (мм)" type="number" val={spec.width} set={v => setSpec({ ...spec, width: +v })} placeholder="220" />
                <Field label="Глубина (мм)" type="number" val={spec.length} set={v => setSpec({ ...spec, length: +v })} placeholder="480" />
              </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer w-fit">
                <input
                  ...
                  className="w-4 h-4 rounded border-gray-300 dark:border-white/20 text-purple-600 focus:ring-purple-500/30"
                />
              ...
              <div className="space-y-2 pt-2">
                  ...
                  {refs.form_factors?.map(f => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          const c = spec.supported_form_factors || [];
                          setSpec({ ...spec, supported_form_factors: sel ? c.filter(i => i !== f.id) : [...c, f.id] });
                          setSpecErrors(p => ({ ...p, supported_form_factors: '' }));
                        }}
                        className={chipClass(sel, err)}
                      >
                        {f.name}
                ...
                {specErrors.supported_form_factors && <p className="text-xs text-red-500 dark:text-red-400">{specErrors.supported_form_factors}</p>}
              </div>
            </div>
          )}
        </div>
