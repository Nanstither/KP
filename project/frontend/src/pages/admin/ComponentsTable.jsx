...
export default function ComponentsTable({
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
      {/* Панель управления */}
      <div className="flex flex-col md:flex-row gap-3 justify-between items-center bg-white dark:bg-[#141416] border border-gray-200 dark:border-white/10 rounded-xl p-3 shadow-sm">
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <select value={stockFilter} onChange={e => { setStockFilter(e.target.value); setPage(1); }} className={`border rounded-lg px-3 py-2 text-sm focus:outline-none ${stockFilter === 'low' ? 'bg-red-50 dark:bg-red-500/10 border-red-300 dark:border-red-500/30 text-red-600 dark:text-red-400' : 'bg-gray-50 dark:bg-[#1b1b1d] border-gray-200 dark:border-white/10 text-gray-900 dark:text-gray-200'}`}>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-white/5">
              {paginated.map(c => {
                ...
                return (
                  <tr key={c.id} className={`hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors ${isEditing ? 'bg-purple-50 dark:bg-purple-500/5' : ''}`}>
                    <td className="px-4 py-3 text-gray-500">#{c.id}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => navigate(`/admin/components/${c.id}/edit`)} className="cursor-pointer text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 font-medium transition-colors">
                        {c.model}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{c.category?.name || '—'}</td>
                    <td className="px-4 py-3">
                      {isEditing ? (
                        <input type="number" min="0" step="100" value={editValues.price} onChange={e => setEditValues({...editValues, price: e.target.value})} className="w-24 bg-gray-50 dark:bg-[#0a0a0c] border border-purple-300 dark:border-purple-500/30 rounded-lg px-2 py-1.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-purple-500 transition-colors" />
                      ) : (
                        <span className="text-purple-600 dark:text-purple-300">{c.price?.toLocaleString('ru-RU')} ₽</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {isEditing ? (
                        <input type="number" min="0" value={editValues.stock} onChange={e => setEditValues({...editValues, stock: e.target.value})} className="w-16 bg-gray-50 dark:bg-[#0a0a0c] border border-purple-300 dark:border-purple-500/30 rounded-lg px-2 py-1.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-purple-500 transition-colors text-center" />
                      ) : (
                        <span className={`px-2 py-1 rounded text-xs font-bold ${
                          c.stock === 0 ? 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400' : 
                          c.stock <= 5 ? 'bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400' : 
                    ...
                    <td className="px-4 py-3 text-right space-x-2">
                      {isEditing ? (
                        ...
                        <>
                          <button onClick={() => startEditing(c)} className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-white transition-colors">
                            <Edit className="w-4 h-4 inline" />
                          </button>
                          <button onClick={() => handleDelete(c.id, c.model)} className="text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors" title="Удалить">
          ...
          <div className="flex items-center justify-between p-4 border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0a0a0c]">
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
              Показывать по: {[10, 25, 50].map(n => (
                <button key={n} onClick={() => { setPerPage(n); setPage(1); }} className={`px-2 py-1 rounded ${perPage === n ? 'bg-purple-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-white/5'}`}>{n}</button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="p-2 rounded-lg border border-gray-200 dark:border-white/10 disabled:opacity-30 hover:bg-gray-200 dark:hover:bg-white/5">&lt;</button>
              <span className="text-sm text-gray-700 dark:text-gray-300 px-2">Стр. {page} из {totalPages}</span>
              <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className="p-2 rounded-lg border border-gray-200 dark:border-white/10 disabled:opacity-30 hover:bg-gray-200 dark:hover:bg-white/5">&gt;</button>
            </div>
          </div>
        )}
      </div>
