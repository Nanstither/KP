...
function ComponentsTab({ components, loading, search, setSearch, categoryFilter, setCategoryFilter, stockFilter, setStockFilter, page, setPage, perPage, setPerPage }) {
      ...
      <div className="flex flex-col md:flex-row gap-3 justify-between items-center bg-[#141416] border border-white/10 rounded-xl p-3">
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input
              type="text" placeholder="Поиск..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
              className="w-full bg-[#0a0a0c] border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-gray-200 focus:border-purple-500/50 focus:outline-none"
            />
          </div>
          
          {/* Фильтр категории */}
          <select value={categoryFilter} onChange={e => { setCategoryFilter(e.target.value); setPage(1); }}
            className="bg-[#0a0a0c] border border-white/10 rounded-lg px-3 py-2 text-sm text-gray-200 focus:outline-none">
          ...
          <select value={stockFilter} onChange={e => { setStockFilter(e.target.value); setPage(1); }}
            className={`border rounded-lg px-3 py-2 text-sm focus:outline-none ${stockFilter === 'low' ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-[#0a0a0c] border-white/10 text-gray-200'}`}>
            <option value="all">Наличие</option>
            <option value="low">⚠️ Заканчивается (5)</option>
            ...
            <tbody className="divide-y divide-white/5">
              {paginated.map(c => (
                <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-purple-300">{c.price?.toLocaleString('ru-RU')} ₽</td>
                  ...
                  <td className="px-4 py-3 text-right space-x-2">
                    <button className="text-gray-400 hover:text-white"><Edit className="w-4 h-4 inline" /></button>
                    <button className="text-gray-400 hover:text-red-400"><Trash2 className="w-4 h-4 inline" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400">
              ...
              {[10, 25, 50].map(n => (
                <button
                  ...
                  className={`px-2 py-1 rounded ${perPage === n ? 'bg-purple-600/30 text-white' : 'hover:bg-white/5'}`}
                >{n}</button>
              ))}
            </div>
            <div className="flex items-center gap-2">
